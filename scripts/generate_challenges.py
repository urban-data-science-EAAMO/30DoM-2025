...
def main() -> int:
    ...
    signup_path = repo_root / "signup" / "30 Days of Mapping Sign-up.xlsx"
    ...
    try:
        ...
        return 1
    try:
        title_col = headers.index(TITLE_HEADER)
    except ValueError:
        print(f"Missing required header: '{TITLE_HEADER}' in row 1")
        return 1
    description_col = headers.index(DESCRIPTION_HEADER) if DESCRIPTION_HEADER in headers else None
    # All other columns (except the three above) are member columns
    member_cols = [i for i, h in enumerate(headers) if i not in {date_col, title_col} and (description_col is None or i != description_col)]
